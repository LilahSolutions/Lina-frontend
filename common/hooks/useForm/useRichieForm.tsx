...
import { INPUT_TYPES, TYPES } from './constants';
import * as validators from './validators';
import Input from '@/common/components/Inputs/Input';
import InputCheckBox from '@/common/components/Inputs/InputCheck/InputCheck';
//textarea
...
interface ValidatorFunction {
  (_value: any, _form?: Form): string | undefined;
}
...
const useRichieForm = (initialFormStructure: FieldData[]) => {
  ...
  const runFieldValidations = (
    ...
    if (isFieldRequired(fieldData) && !checkFieldComplete(value, fieldData)) {
      return validators._isRequired(null); // Get required field error message.
    }
    ...
    const checkFormErrors = (
      ...
      fields.forEach((fieldData) => {
        ...
        if (fieldData.structure?.isObject) {
          ...
          // Map of objects
          Object.entries(formValue).forEach(([key, value]: [any, any]) => {
            errors[fieldData.name][key] = {}; // Create an object to store the errors of each key in the map.
            checkFormErrors(
              fieldData.structure?.elementsType || [],
              value,
              errors[fieldData.name][key]
          ...
          formValue.forEach((value: any, i: any) => {
            errors[fieldData.name].push({}); // Create an object to store the errors of each object in the array.
            checkFormErrors(
              fieldData.structure?.elementsType,
              value,
              errors[fieldData.name][i]
  ...
  const formBuilder = (
    ...
    return formFields.map((input) => {
      ...
      const renderer: Record<string, React.ComponentType<any>> = {
        ...
        [INPUT_TYPES.radio]: InputRadioGroup,
      };

      const Component = renderer[input.type];
      ...
      return (
        <Component
          ...
          handleChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            handleChange(inputFullName, e.target.value)
          }
